document.addEventListener('DOMContentLoaded', function() {
    const dataArrTemp = document.querySelectorAll('.post-data');
    const dataArr = Array.from(dataArrTemp).reverse();
    dataArr.forEach(script => {
        const userId = parseInt(script.dataset.user);
        const postId = parseInt(script.dataset.post); 
        const imagesArr = JSON.parse(script.textContent);
        initSlider(userId, postId, imagesArr);
